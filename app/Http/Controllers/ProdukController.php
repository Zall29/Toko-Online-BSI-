...
{ 
    ...
    { 
        ...
        //  dd($produk);// Debugging untuk melihat data produk
    ...
    { 
        // Mengambil data produk berdasarkan ID dengan relasi fotoProduk
        $produk = Produk::with('fotoProduk')->findOrFail($id); 

        // Mengambil semua data kategori dari tabel Kategori, diurutkan berdasarkan nama kategori secara ascending (A-Z)
        $kategori = Kategori::orderBy('nama_kategori', 'asc')->get(); 

        // Mengembalikan view 'backend.v_produk.show' dengan data berikut:
        // 1. 'judul' berisi teks 'Detail Produk' untuk ditampilkan sebagai judul halaman.
        // 2. 'show' berisi data produk yang diambil dengan relasi fotoProduk.
        // 3. 'kategori' berisi daftar kategori yang sudah diurutkan.
...
// Method untuk mencetak Laporan Produk
public function cetakProduk(Request $request)
{
    ...
    // Tampilkan view cetak laporan produk
...
{
    // Ambil semua foto tambahan untuk produk ini
    $fotoProdukTambahan = FotoProduk::where('produk_id', $id)->get();

    // Ambil detail produk berdasarkan ID
    $detail = Produk::findOrFail($id);

    // Ambil semua kategori, diurutkan berdasarkan nama secara descending
